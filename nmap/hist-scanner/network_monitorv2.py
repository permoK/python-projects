...
class NetworkMonitor:
    def __init__(self, network, interval=300, output_file='network_activity.csv'):
    ...
    def packet_callback(self, packet):
        if IP in packet:
            ...
            if src_ip in self.devices:
                with self.lock:
                    self.devices[src_ip]['activity'][dst_ip] = self.devices[src_ip]['activity'].get(dst_ip, 0) + 1
    ...
    def save_to_csv(self):
        with self.lock:
            with open(self.output_file, 'w', newline='') as file:
                ...
                writer.writerow(['IP', 'MAC', 'First Seen', 'Last Seen', 'Activity'])
                for ip, data in self.devices.items():
                    activity = '; '.join([f"{dst_ip}: {count}" for dst_ip, count in data['activity'].items()])
                    writer.writerow([ip, data['mac'], data['first_seen'], data['last_seen'], activity])
...
def parse_arguments():
    ...
    parser.add_argument("--output", default="network_activity.csv", help="Output CSV file (default: network_activity.csv)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Note: traffic_thread can't be joined as it's a continuous sniff operation
        monitor_thread.join()
