...
# Ask user to input the IP address or domain name they want to scan.
while True:
    ip_or_domain = input("\nPlease enter the IP address or domain name that you want to scan: ")
    ip_add_entered = ip_or_domain
    if ip_add_pattern.search(ip_or_domain):
        print(f"{ip_or_domain} is a valid IP address")
        break
    else:
        resolved_ip = resolve_domain(ip_or_domain)
        if resolved_ip:
            ip_add_entered = resolved_ip
            print(f"{ip_or_domain} resolved to {resolved_ip}")
            break
        else:
            print(f"Could not resolve domain name: {ip_or_domain}")
